...
runBoxTests = do
  ...
  nonce <- generateNonce
  let
    alicePubKey = getBoxPublicKey aliceKp
  let
    aliceSecKey = getBoxSecretKey aliceKp
  let
    bobPubKey = getBoxPublicKey bobKp
  let
    bobSecKey = getBoxSecretKey bobKp
  let
    evilSecKey = getBoxSecretKey evilKp
  let
    str = "Test Message 123"
  let
    msg = fromString str
  -- | Test getting keyPair from BoxSecretKey
  let
    bobKp2 = (getBoxKeyPair bobSecKey)
  ...
  let
    myBox = box msg nonce alicePubKey bobSecKey
  -- | Fail to decrypt box with wrong keys
  let
    msgFail = boxOpen myBox nonce bobPubKey bobSecKey
  assert $ isNothing msgFail
  -- | Succeed to decrypt box with right keys
  let
    msgSuccess = boxOpen myBox nonce alicePubKey bobSecKey
  assert $ cmpMsg str msgSuccess
  -- | Test Before/After Box Encryption/Decryption
  let
    mySharedKey = boxBefore bobPubKey aliceSecKey
  let
    myBadSharedKey = boxBefore alicePubKey evilSecKey
  let
    myBoxAfter = boxAfter msg nonce mySharedKey
  let
    myOpenMsgFail = boxOpenAfter myBoxAfter nonce myBadSharedKey
  let
    myOpenMsgSuccess = boxOpenAfter myBoxAfter nonce mySharedKey
