...
runSecretBoxTests = do
  ...
  let
    str = "Test Message 12345"
  let
    msg = fromString str
  let
    sBox = secretBox msg nonce keyA
  -- Try with good nonce but bad Key
  let
    decMsgFail = secretBoxOpen sBox nonce keyB
  assert $ isNothing decMsgFail
  -- Try with badNonce but good Key
  let
    decMsgFail2 = secretBoxOpen sBox badNonce keyA
  assert $ isNothing decMsgFail
  -- Try with right nonce and key
  let
    decMsgSuccess = secretBoxOpen sBox nonce keyA
